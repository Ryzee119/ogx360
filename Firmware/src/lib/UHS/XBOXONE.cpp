...
XBOXONE::XBOXONE(USB *p) : pUsb(p),          // pointer to USB class instance - mandatory
...
uint8_t XBOXONE::Poll()
{
    ...
    {                                                                       // Do not poll if shorter than polling interval
        ...
        { // Not a matter of no update to send
...
void XBOXONE::readReport()
{
    ...
    if (readBuf[0] != 0x20)
    { // Check if it's the correct report, otherwise return - the controller also sends different status reports
#ifdef EXTRADEBUG
        Notify(PSTR("\r\nXbox Poll: "), 0x80);
