					...
					//if you happen to press this reset combo mid rumble.
						...
						if(XboxOG[i].rumbleUpdate==1){
							...
							switch (commandToggle[i]){
								...
								case 250:
								Xbox360.checkStatus1(i); //The controller responds with it's connection status for the controller.
								XboxOG[i].commandTimer=millis();
								commandToggle[i]++;
								break;
								case 251:
								Xbox360.checkStatus2(i); //The controller responds with a announcement message if connected containing the serial number and battery level.
								XboxOG[i].commandTimer=millis();
								commandToggle[i]++;
								break;
								case 252:
